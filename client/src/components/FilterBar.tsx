...
import { useLogStore, createDefaultFilterV2, countActiveRules } from '../store/logStore';
...
export function FilterBar() {
    ...
    // Get the active view's autoScroll setting and filterV2
    const activeView = views.find(v => v.id === activeViewId);
    const autoScroll = activeView?.autoScroll ?? true;

    // Get or create filterV2 for the active view
    const filterV2 = activeView?.filterV2 ?? createDefaultFilterV2();

    // Count active filter rules
    const activeFilterCount = useMemo(() => {
        if (!filterV2) return 0;
        let total = 0;
        [filterV2.sessions, filterV2.levels, filterV2.appNames, filterV2.hostNames, filterV2.titles, filterV2.entryTypes].forEach(fr => {
    ...
    return (
        <div className={`${density.barHeight} bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 ${density.px} flex items-center ${density.gap} flex-shrink-0`}>
            ...
            {/* Active filters badge (informational - use Filters tab on grid to edit) */}
            {activeFilterCount > 0 && (
                <div className="flex items-center gap-1 px-2 py-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 rounded text-xs">
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z" />
                    </svg>
                    <span className="font-medium">{activeFilterCount} filter{activeFilterCount !== 1 ? 's' : ''}</span>
                </div>
            )}
